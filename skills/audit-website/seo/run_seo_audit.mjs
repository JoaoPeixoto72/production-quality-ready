...
const SKILL_DIR = SCRIPT_DIR // seo/ sub-engine of audit-website: audit.config.json and profiles/ live beside the script
...
if (!targetUrl && !buildDir) {
  console.log('Usage:')
  console.log('  node run_seo_audit.mjs --url=<https-url>')
...
async function runLiveAudit(url) {
  ...
  if (psiKey) {
    console.log('\n--- CORE WEB VITALS (PSI) ---')
    try {
      const psiUrl = `${PSI_ENDPOINT}?${new URLSearchParams({ url, strategy: 'mobile', category: 'performance', key: psiKey })}`
...
const OUT_DIR = getFlag('out-dir') || ROOT
fs.mkdirSync(OUT_DIR, { recursive: true })
fs.writeFileSync(path.join(OUT_DIR, 'seo-audit-report.json'), JSON.stringify(report, null, 2), 'utf8')
...
fs.writeFileSync(path.join(OUT_DIR, 'seo-audit-report.sarif'), JSON.stringify(sarif, null, 2), 'utf8')
