---
name: skill-auditor
description: "Audit Agent Skills (Claude Code, Antigravity, Codex CLI) against POLICY.md — frontmatter, description discrimination, permissions, declared resources, verifiable claims, bilateral pair-check between owners that touch each other. Combines a mechanical linter with semantic review by model profile. Use before commit/release of a skill. Do NOT use to audit application code, a feature PR, or general docs — that's audit-app. Read-only."
...
- Run `bash scripts/audit.sh <path>` — deterministic, ~ms. **This list is the
  ...
  - **frontmatter is valid YAML** (§1.b) — a real `yaml.safe_load` accepts it; a `: ` in an unquoted description silently breaks agents that parse it
...
- It does **not** discover `SKILL.md` files under `fixtures/`, `tests/` or `.git/`:
  ...
  hostile fixture (`references/fixtures/hostile-skill/`) and the invalid-yaml
  fixture (`references/fixtures/invalid-yaml-skill/`) double as this script's
  own regression tests; each should produce at least one Blocker when linted
  directly.
- §1.b (valid YAML) requires `python3` on PATH. If it is missing, the check is
  silently skipped — the linter says so in the output header, so an operator
  never reads "passed" as "was checked". A skill shipping to an agent runtime
  will always have Python available (the plugin's own scripts assume it), so
  the skip is a compatibility escape hatch, not a design gap.
