...
check_skill() {
  ...
  # §1.b Frontmatter is valid YAML — a `:` followed by a space inside an
  # unquoted string starts a new key mid-value ("Rule: OWASP" reads as
  # {Rule: {OWASP: ...}}). The earlier check ("has a description:") was regex
  # and did not notice — five descriptions shipped broken to agents that
  # parse frontmatter with a real YAML loader. This is a Blocker: an unparsed
  # frontmatter means the skill's own routing is invisible to the caller.
...
try:
    ...
    if not isinstance(data, dict):
        print("frontmatter not a mapping")
        sys.exit(1)
except yaml.YAMLError as e:
    print(str(e).split("\n")[0])
    sys.exit(1)
' <<<"$fm" 2>&1)" || {
      emit Blocker "$skill_md: frontmatter is not valid YAML — $yaml_err (§1.b: a real YAML loader is what agents actually use; if this fails, the skill's routing is invisible to them)"
    }
  fi
...
if ! command -v python3 >/dev/null 2>&1; then
  echo "Note: python3 not on PATH — §1.b (YAML validity) skipped for all targets."
fi
